  ...
  def main(args: Array[String]): Unit = {
    ...
    val STUDENT_TEMP2 = student.
      select($"student_id".as("id"),
        trim(concat($"branch", lit("_"), translate(trim($"college"), lit(Regex), lit("")))).as("branch_college")).
      distinct
    ...
    val STUDENT_TEMP5 = student.
      select($"student_id".as("id"),
        trim(concat($"branch", lit("_"), translate(trim($"college"), lit(Regex), lit("")))).cast("char (64)").as("branch_college")).
      distinct
