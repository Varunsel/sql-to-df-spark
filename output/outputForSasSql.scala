...
val STUDENT_TEMP3 =
    spark.sql(s"""            select distinct student_id as id,trim(  
 CONCAT(branch,'_',translate(trim(college),Regex,''))) as branch_college        from
        	student
         where 
...
val STUDENT_TEMP5 =
    spark.sql(s"""        select             	distinct student_id as id, 
 CAST(trim(
CONCAT(branch,'_',translate(trim(college),Regex,''))) as char(64) )  as branch_college             from                student""")
...
val STUDENT_TEMP6 =
    spark.sql(s"""        select         	distinct student_id as id, 
 CAST((
CONCAT(branch,'_','${postfix}')) as char(64) )  as branch_college         from            student""")
